...
def parse_arguments():
    ...
    parser.add_argument('-p', '--proxys', type=str, help="File containing a list of proxies")
    ...
    proxies = load_proxies_from_file(args.proxys) if args.proxys else None

    return users, passwords, args.tor, proxies
...
if __name__ == '__main__':
    users, passwords, use_tor, proxies = parse_arguments()
    ...
    if proxies:
        print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Using Proxies:")
        for proxy in proxies:
            print(f"{Fore.LIGHTBLUE_EX}{proxy}{Fore.RESET}")
    ...
    while current_password_index < len(passwords):
        for user in users:
            ...
            for i in range(current_password_index, current_password_index + password_limit):
                ...
                if proxies and attempt_count % 10 == 0:
                    # Use a new proxy every 10 attempts
                    proxy = proxies[current_proxy_index % len(proxies)]
                    current_proxy_index += 1
                    print(
                        f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Using Proxy: {Fore.LIGHTBLUE_EX}{proxy}")
                ...
                if attempt_count % 49 == 0:
                    ...
                    print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET}Waiting for 11 minutes...")
                    time.sleep(11 * 60)
